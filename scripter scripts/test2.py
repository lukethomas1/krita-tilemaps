...
def generate_default_mask():
    ...
    for loc in outer_corner_locations:
        clones.append(clone_move_rotate(mask_outer_corner, loc, group_layer))
    for loc in edge_connector_locations:
        clones.append(clone_move_rotate(mask_edge_connector, loc, group_layer))
    for loc in inner_corner_locations:
        clones.append(clone_move_rotate(mask_inner_corner, loc, group_layer))
    for loc in border_locations:
        clones.append(clone_move_rotate(mask_border, loc, group_layer))
    for loc in mask_fill_locations:
        clones.append(clone_move_rotate(mask_fill, loc, group_layer))

def clone_move_rotate(node, loc, group_layer) -> Node:
    doc = Krita.instance().activeDocument()
